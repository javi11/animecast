...
@Component({
    ...
    template: `
        <div class="container">
            ...
            <ion-select  [(ngModel)]="trackSelected" (ionChange)="selectTrack($event)">
            <ion-option *ngFor="let track of tracks"  [value]="track.id" [selected]="track.selected === true">{{ track.label }}</ion-option>
          </ion-select>
        </div>
    `,
    styles: [ `
        ...
        track-selector select.trackSelector {
            width: 50px;
            padding: 5px 8px;
            border: none;
            background: none;
        ...
        track-selector select.trackSelector:focus {
            outline: none;
        }
        track-selector .track-selected {
            position: absolute;
            width: 100%;
            text-align: center;
        ...
        track-selector .vg-icon-closed_caption:before {
            width: 100%;
        }
    ` ]
})
