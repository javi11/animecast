...
export class EpisodeService {
  ...
  updateEpisodeStatus(showLink: string, episode: any): Promise<any> {
    return this.getEpisodes(showLink).then((episodes) => {
      ...
      return this.config.get().then(config => this.storage.ready().then(() => this.storage.set(`${config.provider}@${config.showLink}`, episodes)));
    })

  }

  getEpisodes(showLink: any): Promise<any> {
    return this.config.get().then(config => this.storage.ready().then(() => this.storage.get(`${config.provider}@${config.showLink}`)));
  }
